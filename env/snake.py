...
class SnakeEnv(gym.Env):
    def __init__(self, grid_height, grid_width, max_steps, render_mode=None):
        ...
        self.observation_space = gym.spaces.Box(low=0, high=1, shape=(grid_height, grid_width, 3))
        self.grid_height = grid_height
        self.grid_width = grid_width
        self.max_steps = max_steps
        self.render_mode = render_mode
        self.ALL_GRID_POSITIONS = set(itertools.product(range(self.grid_height), range(self.grid_width)))

    def _get_valid_food_position(self):
        possible =  self.ALL_GRID_POSITIONS \
    ...
    def reset(self):
        ...
        snake_position = (np.random.randint(0, self.grid_height), np.random.randint(0, self.grid_width))
        ...
        return self.grid, {'snake_positions': self.snake_positions, 'food_position': self.food_position}
    ...
    def step(self, action):
        ...
        return self.grid, 1 if has_eaten else -0.001, False, self.env_step >= self.max_steps, info
    ...
    def render(self, mode='rgb_array'):
        ...
        cv2.putText(img, score_text, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
